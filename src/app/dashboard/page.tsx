                        ...
                        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
                            <div className="space-y-6">
                                ...
                                {activeVehicle.recalls.length > 0 && (
                                    <section className="rounded-3xl border border-red-500/40 bg-red-600/10 p-6 shadow-xl shadow-red-950/20">
                                        <div className="mb-4 flex items-center gap-3">
                                            <span className="flex h-9 w-9 items-center justify-center rounded-xl border border-red-500/50 bg-red-600/30 text-base font-black text-red-100">!</span>
                                            <div>
                                                <p className="text-sm font-semibold uppercase tracking-[0.32em] text-red-300">Open recalls</p>
                                                <h3 className="mt-1 text-xl font-black text-white">Action required before next visit</h3>
                                            </div>
                                        </div>
                                        <ul className="space-y-3">
                                            {activeVehicle.recalls.map((recall) => (
                                                <li key={recall.campaign} className="rounded-2xl border border-red-500/30 bg-zinc-950/70 p-5">
                                                    <div className="flex flex-wrap items-center gap-3">
                                                        <span className="rounded-full border border-red-500/50 bg-red-600/20 px-3 py-1 text-xs font-bold uppercase tracking-[0.22em] text-red-200">Campaign {recall.campaign}</span>
                                                        <span className="text-sm font-semibold text-red-200">{recall.remedy}</span>
                                                    </div>
                                                    <p className="mt-3 text-lg font-bold text-white">{recall.component}</p>
                                ...
                                <section className="rounded-3xl border border-zinc-800 bg-zinc-900 p-6">
                                    <div className="mb-5 flex items-end justify-between">
                                        <div>
                                            ...
                                            <h3 className="mt-2 text-2xl font-black text-white">Recent visits</h3>
                                        </div>
                                        <span className="rounded-full border border-zinc-700 bg-zinc-950 px-3 py-1 text-xs font-semibold uppercase tracking-[0.22em] text-zinc-400">
                                            {activeVehicle.serviceHistory.length} record{activeVehicle.serviceHistory.length === 1 ? '' : 's'}
                                        </span>
                                    </div>
                                    {activeVehicle.serviceHistory.length === 0 ? (
                                        <p className="rounded-2xl border border-dashed border-zinc-800 bg-zinc-950 px-5 py-6 text-center text-sm text-zinc-400">No service history on file for this vehicle yet.</p>
                                    ) : (
                                        <ul className="space-y-3">
                                            {activeVehicle.serviceHistory.map((visit) => {
                                                const isOpen = Boolean(expandedRows[visit.id]);
                                                const hasDeclined = visit.declined.length > 0;
                                                return (
                                                    <li key={visit.id} className="overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950">
                                                        <button type="button" onClick={() => toggleHistoryRow(visit.id)} className="flex w-full flex-col gap-3 px-5 py-4 text-left transition hover:bg-zinc-900 sm:flex-row sm:items-center sm:justify-between">
                                                            <div className="flex flex-col gap-1">
                                                                <span className="text-xs font-semibold uppercase tracking-[0.24em] text-zinc-500">{visit.date}</span>
                                                                <span className="text-base font-bold text-white">{visit.services.join(' · ')}</span>
                                                                <span className="text-xs text-zinc-400">Tech: {visit.techName}</span>
                                                            </div>
                                                            <div className="flex items-center gap-3">
                                                                {hasDeclined && (
                                                                    <span className="rounded-full border border-amber-500/40 bg-amber-500/10 px-3 py-1 text-xs font-bold uppercase tracking-[0.2em] text-amber-200">Declined work</span>
                                                                )}
                                                                <span className="text-lg font-black text-zinc-100">{formatCurrency(visit.cost)}</span>
                                                                <span className={`text-xs font-bold uppercase tracking-[0.2em] text-zinc-500 transition ${isOpen ? 'rotate-90' : ''}`}>›</span>
                                                            </div>
                                                        </button>
                                                        {isOpen && (
                                                            <div className="border-t border-zinc-800 bg-zinc-950/60 px-5 py-4">
                                                                <div className="grid gap-4 sm:grid-cols-2">
                                                                    <div>
                                                                        <p className="text-xs font-semibold uppercase tracking-[0.24em] text-zinc-500">Services performed</p>
                                                                    ...
                                                                    <div>
                                                                        <p className="text-xs font-semibold uppercase tracking-[0.24em] text-zinc-500">Declined recommendations</p>
                                                                        {hasDeclined ? (
                                                                            <ul className="mt-2 space-y-2">
                                                                                {visit.declined.map((item) => (
                                                                                    <li key={item.service} className="rounded-xl border border-amber-500/30 bg-amber-500/5 px-3 py-2 text-sm text-amber-100">
                                                                                        <div className="flex items-center justify-between gap-3">
                                                                                            <span className="font-bold">{item.service}</span>
                                                                                            <span className="text-sm font-bold text-amber-200">{formatCurrency(item.cost)}</span>
                                                                                        </div>
                                                                            ...
                                                                            <p className="mt-2 text-sm text-zinc-400">No declined work on this visit.</p>
                                                                        )}
                                                                    </div>
                                                                </div>
                                                            </div>
                            ...
                            <aside className="space-y-6">
                                <section className="rounded-3xl border border-zinc-800 bg-zinc-900 p-6">
                                    ...
                                    <h3 className="mt-2 text-xl font-black text-white">Live capacity</h3>
                                    {(() => {
                                        const capacity = data.shopStatus.capacityPercent;
                                        const palette = getCapacityColor(capacity);
                                        return (
                                            <div className="mt-5">
                                                <div className="flex items-baseline justify-between">
                                                    <span className={`text-4xl font-black ${palette.text}`}>{capacity}%</span>
                                                    <span className={`rounded-full border px-3 py-1 text-xs font-bold uppercase tracking-[0.2em] ${palette.pill}`}>
                                                        {capacity > 80 ? 'At limit' : capacity >= 60 ? 'Busy' : 'Open'}
                                                    </span>
                                                </div>
                                                <div className="mt-3 h-2 w-full overflow-hidden rounded-full bg-zinc-800">
                                                    <div className={`h-full ${palette.bar}`} style={{ width: `${Math.min(100, Math.max(0, capacity))}%` }} />
                                                </div>
                                                <dl className="mt-5 grid grid-cols-2 gap-3 text-sm">
                                                    <div className="rounded-2xl border border-zinc-800 bg-zinc-950 p-3">
                                                        <dt className="text-xs font-semibold uppercase tracking-[0.22em] text-zinc-500">Est. wait</dt>
                                                        <dd className="mt-1 text-lg font-black text-white">{data.shopStatus.estimatedWaitMinutes} min</dd>
                                                    </div>
                                                    <div className="rounded-2xl border border-zinc-800 bg-zinc-950 p-3">
                                                        <dt className="text-xs font-semibold uppercase tracking-[0.22em] text-zinc-500">Technicians</dt>
                                                        <dd className="mt-1 text-lg font-black text-white">{data.shopStatus.technicians}</dd>
                                                    </div>
                                                    <div className="col-span-2 rounded-2xl border border-zinc-800 bg-zinc-950 p-3">
                                                        <dt className="text-xs font-semibold uppercase tracking-[0.22em] text-zinc-500">Open repair orders</dt>
                                                        <dd className="mt-1 text-lg font-black text-white">{data.shopStatus.openRepairOrders}</dd>
                                                    </div>
                                                </dl>
                                            </div>
                                        );
                                ...
                                <section className="rounded-3xl border border-zinc-800 bg-zinc-900 p-6">
                                    ...
                                    <h3 className="mt-2 text-xl font-black text-white">{data.nextAppointment.date}</h3>
                                    <p className="mt-1 text-sm font-semibold text-zinc-300">{data.nextAppointment.time}</p>
                                    <ul className="mt-4 space-y-2">
                                        {data.nextAppointment.services.map((service) => (
                                            <li key={service} className="flex items-center gap-2 rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm text-zinc-200">
                                                <span className="h-1.5 w-1.5 rounded-full bg-red-500" />
                                                {service}
                                            </li>
                                        ))}
                                    </ul>
                                    <button type="button" className="mt-5 w-full rounded-2xl border border-red-500/40 bg-red-600/15 px-4 py-3 text-sm font-bold uppercase tracking-[0.22em] text-red-200 transition hover:bg-red-600/25">
