...
import { enrichMockUpsells, enrichSupabaseUpsells, type UpsellWithCustomerContext } from '@/lib/upsell-context';
...
function getMockSummary(today: string) {
    ...
    const upsells = enrichMockUpsells(rawUpsells);
...
export async function GET(request: Request) {
    ...
    if (!supabase) {
        return NextResponse.json({ persistence: 'none' as const, today, arrivals: [] as AppointmentRow[], loaner_queue: [] as LoanerRequestRow[], upsells: [] as UpsellWithCustomerContext[], stats: { arrivals_count: 0, loaner_pending: 0, upsell_count: 0, upsell_value: 0 } });
    }
    ...
    const upsells = await enrichSupabaseUpsells(supabase, (upsellsRes.data ?? []) as UpsellRow[]);
